...
- User prefs (selected preset, custom time control fields, sound, player names, rotate mode) persist to `localStorage` under `chessclockonline:prefs` — always update `persist()` when adding a new preference field.
